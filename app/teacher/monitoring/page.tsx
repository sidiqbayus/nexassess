  ...
  return (
    ...
    >
       ...
       <div className="p-3 md:p-4 border-t border-slate-100 bg-white flex flex-col gap-2">
          <div className={`flex items-center justify-between text-[8px] md:text-[9px] font-bold uppercase tracking-widest p-1.5 md:p-2 rounded-lg border ${isDeviceLocked ? 'bg-indigo-50/50 border-indigo-100 text-indigo-600' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
...
export default function TeacherMonitoringPage() {
  ...
  if (loading) {
    return <div className="py-20 flex justify-center"><LoaderCircle className="w-10 h-10 md:w-12 md:h-12 text-indigo-500 animate-spin" /></div>;
  }

  return (
    <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 p-4 md:p-8 max-w-7xl mx-auto text-slate-900 relative pb-24 md:pb-8">
         ...
         <div className="fixed inset-0 z-[300] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 animate-in fade-in">
            <div className="bg-white w-full max-w-lg rounded-2xl md:rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200">
               <div className="p-4 md:p-6 lg:p-8 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                  <h2 className="text-lg md:text-xl font-black text-slate-800 flex items-center gap-2 md:gap-3"><Radio className="w-5 h-5 md:w-6 md:h-6 text-indigo-600"/> Siarkan Pengumuman</h2>
               ...
               <div className="p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6">
                  <div>
                     ...
                     <div className="relative">
                       <select value={broadcastTarget} onChange={(e) => setBroadcastTarget(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-3 md:px-4 py-3 md:py-3.5 text-xs md:text-sm font-bold text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none appearance-none cursor-pointer shadow-sm">
                  ...
                  {broadcastTarget === 'room' && (
                     <div className="animate-in fade-in slide-in-from-top-2 relative">
                        <select value={broadcastTargetId} onChange={(e) => setBroadcastTargetId(e.target.value)} className="w-full bg-indigo-50/50 border border-indigo-200 rounded-xl px-3 md:px-4 py-3 md:py-3.5 text-xs md:text-sm font-bold text-indigo-700 focus:ring-2 focus:ring-indigo-500 outline-none appearance-none cursor-pointer">
                           <option value="">-- Pilih Ruangan Spesifik --</option>
                           {rooms.map(rm => <option key={rm.id} value={rm.id}>{rm.room_name} ({rm.subject})</option>)}
                        </select>
                        <ChevronDown className="absolute right-3 md:right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-indigo-500 pointer-events-none" />
                     </div>
                  )}
                  <div>
                     ...
                     <textarea value={broadcastText} onChange={(e) => setBroadcastText(e.target.value)} rows={4} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 md:px-4 py-3 md:py-3.5 text-xs md:text-sm font-medium text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none resize-none placeholder:text-slate-400 shadow-inner" placeholder="Ketik pesan yang akan dimunculkan secara paksa di layar siswa..."></textarea>
                  </div>
               </div>
               <div className="p-4 md:p-6 lg:p-8 border-t border-slate-100 bg-slate-50 flex gap-2 md:gap-3">
                  ...
                  <button onClick={executeBroadcast} disabled={!broadcastText || (broadcastTarget === 'room' && !broadcastTargetId)} className="flex-1 py-3 md:py-3.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-md shadow-indigo-200 disabled:bg-slate-300 disabled:shadow-none flex items-center justify-center gap-1.5 md:gap-2 active:scale-95 transition-all text-xs md:text-sm"><Send className="w-3.5 h-3.5 md:w-4 md:h-4"/> Siarkan</button>
               </div>
            </div>
         </div>
      )}
             ...
             <div className="p-4 md:p-5 lg:p-8 border-b border-slate-100 bg-slate-50/80 flex justify-between items-start sm:items-center shrink-0">
                <div className="flex items-center gap-3 sm:gap-4 md:gap-5 min-w-0">
                   {detailStudentObj.avatar_url ? (
                      ...
                      <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-indigo-100 flex items-center justify-center text-indigo-600 border border-indigo-200 shadow-sm shrink-0"><UserCircle2 className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7"/></div>
                   )}
                   <div className="min-w-0 flex-1">
                      <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-black text-slate-800 flex flex-wrap items-center gap-2 md:gap-3 leading-tight">
                         ...
                         {detailIsBlockedBySystem ? (
                            ...
                            <span className="text-[8px] md:text-[10px] bg-indigo-100 text-indigo-700 px-1.5 md:px-2.5 py-0.5 md:py-1 rounded md:rounded-md border border-indigo-200 font-black animate-pulse tracking-widest uppercase whitespace-nowrap">AKTIF</span>
                         )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
                 ...
                 <div className="space-y-4 sm:space-y-6">
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl md:rounded-[1.5rem] p-4 md:p-6 shadow-sm">
                       <h3 className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest mb-3 md:mb-5 flex items-center gap-1.5 md:gap-2"><Server className="w-3.5 h-3.5 md:w-4 md:h-4 text-indigo-500"/> Identitas Perangkat</h3>
                       <div className="space-y-3 md:space-y-4 text-xs md:text-sm">
                          <div className="flex flex-col border-b border-slate-100 pb-2 md:pb-3">
                             ...
                             <span className="font-mono font-black text-indigo-700 text-sm md:text-base">{modalIp}</span>
                          </div>
                          <div className="flex flex-col border-b border-slate-100 pb-2 md:pb-3">
                             ...
                             return (
                                <div key={opt.key} className="flex items-center justify-between p-2.5 md:p-3.5 rounded-lg md:rounded-xl bg-slate-50 border border-slate-200 shadow-sm">
                                   <div className="flex items-center gap-2 md:gap-3">
                                      <Icon className={`w-3.5 h-3.5 md:w-4 md:h-4 shrink-0 ${!isBypassed ? 'text-indigo-600' : 'text-slate-400'}`}/>
                                   ...
                                   <button 
                                      ...
                                      className={`relative inline-flex h-5 w-9 md:h-6 md:w-11 shrink-0 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 ${!isBypassed ? 'bg-indigo-600 shadow-inner' : 'bg-slate-300'}`}
                                   >
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl md:rounded-[1.5rem] p-4 md:p-6 shadow-sm">
                       ...
                       <div className="space-y-3 md:space-y-4">
                          <button onClick={() => triggerResetDevice(detailSessionObj.id, detailStudentObj.full_name)} className="w-full flex items-center justify-between p-3 md:p-4 bg-white hover:bg-indigo-50 border border-slate-200 hover:border-indigo-300 rounded-lg md:rounded-xl transition-all shadow-sm group">
                             <div className="flex items-center gap-2 md:gap-3 text-left">
                                <div className="p-1.5 md:p-2 bg-indigo-100 text-indigo-600 rounded-md md:rounded-lg group-hover:bg-indigo-600 group-hover:text-white transition-colors"><Smartphone className="w-3.5 h-3.5 md:w-4 md:h-4"/></div>
                                <div>
                                   <p className="text-xs md:text-sm font-black text-slate-800 group-hover:text-indigo-700 transition-colors">Reset Kunci Perangkat</p>
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl md:rounded-[1.5rem] p-4 md:p-6 shadow-sm">
                       <h3 className="text-[10px] md:text-xs font-black text-indigo-600 uppercase tracking-widest mb-3 md:mb-4 flex items-center gap-1.5 md:gap-2"><MessageSquare className="w-3.5 h-3.5 md:w-4 md:h-4"/> Pesan Personal Screen</h3>
                       <div className="flex flex-col gap-2.5 md:gap-3">
                          <textarea value={dmText} onChange={e => setDmText(e.target.value)} rows={3} className="w-full bg-slate-50 border border-slate-200 rounded-lg md:rounded-xl px-3 md:px-4 py-2.5 md:py-3 text-xs md:text-sm font-bold text-slate-800 focus:border-indigo-500 focus:ring-2 focus:bg-white outline-none resize-none placeholder-slate-400 shadow-inner" placeholder={`Ketik pesan untuk ${detailStudentObj.full_name}...`}></textarea>
                          <button onClick={() => sendDirectMessage(detailStudentObj.id, detailStudentObj.full_name)} disabled={!dmText.trim()} className="w-full py-2.5 md:py-3.5 bg-indigo-600 text-white font-bold text-xs md:text-sm rounded-lg md:rounded-xl hover:bg-indigo-700 transition-all disabled:bg-slate-300 disabled:shadow-none shadow-md shadow-indigo-200 flex justify-center items-center gap-1.5 md:gap-2 active:scale-95"><Send className="w-3.5 h-3.5 md:w-4 md:h-4"/> Kirim Pesan</button>
                       </div>
                    </div>
                 ...
                 <div className="lg:col-span-2 space-y-4 sm:space-y-6 md:space-y-8">
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl md:rounded-[1.5rem] p-4 md:p-6 lg:p-8 shadow-sm">
                       <div className="flex justify-between items-center mb-3 md:mb-5 pb-3 md:pb-5 border-b border-slate-100">
                          ...
                          {locationLog && (
                             <a href={`http://googleusercontent.com/maps.google.com/maps?q=${locationLog.snapshot_url.split(',')[0]},${locationLog.snapshot_url.split(',')[1]}`} target="_blank" rel="noreferrer" className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 px-2.5 py-1.5 md:px-3 md:py-2 rounded-lg flex items-center gap-1 md:gap-1.5 transition-colors shadow-sm whitespace-nowrap">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
                       ...
                       <div className="bg-white border border-slate-200 rounded-xl md:rounded-[1.5rem] p-4 md:p-6 shadow-sm">
                          <div className="flex items-center justify-between mb-4 md:mb-5 pb-3 md:pb-5 border-b border-slate-100">
                            <h3 className="text-xs md:text-sm font-black text-slate-800 flex items-center gap-1.5 md:gap-2"><Camera className="w-4 h-4 md:w-5 md:h-5 text-indigo-500"/> Snap Kamera</h3>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 md:gap-6 bg-white p-5 md:p-6 lg:p-8 rounded-2xl md:rounded-[2.5rem] border border-indigo-100 shadow-lg relative overflow-hidden">
        <div className="absolute top-0 left-0 w-full h-1.5 md:h-2.5 bg-gradient-to-r from-indigo-600 to-purple-600"></div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
             <div className="relative flex-1 sm:w-64 md:w-80">
                <select 
                   ...
                   className="w-full bg-white border border-slate-200 rounded-xl md:rounded-2xl px-3 md:px-5 py-3 md:py-4 text-xs md:text-sm font-black text-slate-800 focus:ring-2 focus:ring-indigo-500 outline-none appearance-none cursor-pointer shadow-sm transition-all hover:border-indigo-300 truncate pr-8"
                >
                   {exams.length === 0 ? <option value="">Tidak ada jadwal aktif</option> : null}
                   {exams.map(ex => <option key={ex.id} value={ex.id}>{ex.subject} - {ex.grade_level || 'Umum'}</option>)}
                </select>
                <ChevronDown className="absolute right-3 md:right-5 top-1/2 -translate-y-1/2 w-4 h-4 md:w-5 md:h-5 text-indigo-500 pointer-events-none font-bold" />
             </div>
             <button onClick={() => fetchMonitoringData(selectedExamId)} disabled={refreshing || !selectedExamId} className="p-3 md:p-4 bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-xl md:rounded-2xl border border-indigo-200 hover:border-indigo-600 transition-all shadow-sm shrink-0 active:scale-95 disabled:opacity-50" title="Segarkan Data Manual">
      ...
      <div className="bg-white p-2.5 md:p-3 rounded-xl md:rounded-[1.5rem] border border-slate-200 shadow-sm flex flex-col md:flex-row items-stretch md:items-center gap-2 md:gap-3 w-full">
         <div className="relative w-full md:flex-1">
            ...
            <input 
              ...
              className="w-full bg-slate-50 border-none rounded-lg md:rounded-xl pl-10 md:pl-11 pr-3 md:pr-4 py-2.5 md:py-3 text-xs md:text-sm font-bold text-slate-800 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all placeholder-slate-400" 
            />
         </div>
         <div className="w-px h-8 bg-slate-200 hidden md:block mx-1"></div>
         <div className="flex flex-col sm:flex-row w-full md:w-auto gap-2">
            <select value={filterType} onChange={(e) => { setFilterType(e.target.value as any); setFilterValue(''); }} className="flex-1 md:w-40 bg-slate-50 border border-slate-200 rounded-lg md:rounded-xl px-3 md:px-4 py-2.5 md:py-3 text-xs md:text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer appearance-none transition-all hover:bg-white">
              <option value="all">Semua Peserta</option>
              <option value="class">Filter Kelas</option>
              <option value="room">Filter Ruangan</option>
            </select>
            {filterType === 'class' && (
               <select value={filterValue} onChange={(e) => setFilterValue(e.target.value)} className="flex-1 md:w-48 bg-white border border-indigo-200 rounded-lg md:rounded-xl px-3 md:px-4 py-2.5 md:py-3 text-xs md:text-sm font-black text-indigo-700 outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer appearance-none shadow-sm animate-in fade-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
         ...
         <div className="bg-white p-4 md:p-8 rounded-xl md:rounded-[2rem] border border-indigo-200 shadow-[0_0_20px_rgba(79,70,229,0.15)] flex flex-col sm:flex-row items-center sm:items-start md:items-center gap-3 md:gap-5 transform hover:-translate-y-1 transition-all duration-300 text-center sm:text-left">
            <div className="w-10 h-10 md:w-14 md:h-14 bg-indigo-500 rounded-xl md:rounded-2xl flex items-center justify-center border border-indigo-400 shrink-0 relative shadow-inner">
               <span className="absolute -top-1 -right-1 flex h-3 w-3 md:h-4 md:w-4"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-300 opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 md:h-4 md:w-4 bg-indigo-400 border border-white"></span></span>
               <Activity className="w-5 h-5 md:w-7 md:h-7 text-white"/>
            </div>
            <div><p className="text-[9px] md:text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-0.5 md:mb-1">Sedang Mengerjakan</p><p className="text-xl md:text-3xl font-black text-indigo-700">{pesertaAktif}</p></div>
         </div>
         <div className="bg-white p-4 md:p-8 rounded-xl md:rounded-[2rem] border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center sm:items-start md:items-center gap-3 md:gap-5 hover:shadow-md transition-shadow text-center sm:text-left">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 md:gap-6 mt-4 md:mt-0">
         {filteredSessions.length === 0 ? (
            <div className="col-span-full py-16 md:py-24 text-center bg-white border border-slate-200 border-dashed rounded-[2rem] md:rounded-[2.5rem] shadow-sm px-4">
               ...
               <h3 className="text-xl md:text-2xl font-black text-slate-700 mb-1.5 md:mb-2">Tidak Ada Peserta Terpantau</h3>
               <p className="text-slate-500 font-medium text-xs md:text-sm">Peserta belum masuk, filter pencarian kosong, atau Anda tidak memiliki akses ke ujian/ruangan ini.</p>
            </div>
         ) : (
