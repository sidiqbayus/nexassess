...
export default function RoomsManagementPage() {
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 p-4 md:p-8 max-w-7xl mx-auto text-slate-900 relative pb-24">
        ...
        <div className="fixed inset-0 z-[120] bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm rounded-2xl md:rounded-[2rem] p-6 md:p-8 shadow-2xl border border-slate-200 animate-in zoom-in-95">
            ...
            <div className="flex items-center gap-2 md:gap-3">
              <button onClick={() => setConfirmDialog(null)} className="flex-1 py-3 md:py-3.5 rounded-xl text-xs md:text-sm font-bold text-slate-600 bg-white border border-slate-200 hover:bg-slate-50 transition-all shadow-sm">Batal</button>
              <button onClick={() => { confirmDialog.onConfirm(); setConfirmDialog(null); }} className={`flex-1 py-3 md:py-3.5 rounded-xl text-xs md:text-sm font-bold text-white transition-all shadow-md active:scale-95 ${confirmDialog.type === 'danger' ? 'bg-rose-600 hover:bg-rose-700 shadow-rose-200' : 'bg-amber-500 hover:bg-amber-600 shadow-amber-200'}`}>
                 Ya, Lanjutkan
              </button>
            </div>
          </div>
         ...
         <div className="fixed inset-0 z-[130] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-md rounded-2xl md:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col border border-slate-100">
               ...
               <div className="p-5 md:p-8 space-y-3 md:space-y-4">
                  <button onClick={executePrintAllPDF} disabled={isGeneratingPdf} className="w-full flex items-center p-4 md:p-5 rounded-xl md:rounded-2xl border-2 border-slate-200 hover:border-blue-500 hover:bg-blue-50 transition-all group text-left disabled:opacity-70 disabled:cursor-not-allowed">
                  ...
                  <button onClick={executeDownloadAllExcel} className="w-full flex items-center p-4 md:p-5 rounded-xl md:rounded-2xl border-2 border-slate-200 hover:border-emerald-500 hover:bg-emerald-50 transition-all group text-left">
      ...
      {/* ================= MODAL IMPORT EXCEL ================= */}
      {isImportOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 md:p-8 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-4xl rounded-2xl md:rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[95vh] md:max-h-[90vh]">
            <div className="p-4 md:p-6 lg:p-8 border-b border-slate-100 bg-slate-50/80 flex justify-between items-center shrink-0">
               <h3 className="text-lg md:text-xl font-black text-slate-800 flex items-center gap-2 md:gap-3 truncate"><FileUp className="w-5 h-5 md:w-6 md:h-6 text-emerald-500 shrink-0"/> <span className="truncate">Import Ruangan & Plot Siswa</span></h3>
            ...
            <div className="p-4 md:p-6 lg:p-8 overflow-y-auto space-y-4 md:space-y-6 custom-scrollbar flex-1">
               <div className="bg-blue-50 border border-blue-100 rounded-xl md:rounded-2xl p-4 md:p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-6 shadow-sm">
                  <div className="flex items-start gap-3 md:gap-4">
                     ...
                     <div>
                       <h4 className="font-bold text-blue-900 mb-1 text-base md:text-lg">Panduan Import Sesi Ruangan Sekaligus Siswa</h4>
                       <p className="text-[10px] md:text-sm text-blue-800/80 mb-2 md:mb-3 leading-relaxed font-medium">
                         ...
                         <span className="text-rose-600 font-medium mt-1 inline-block">Baris merah = Nama Ruangan DAN Mapel sudah ada di database (Akan ditolak).</span>
                       </p>
                     </div>
                  </div>
               ...
               <label className={`border-2 border-dashed rounded-xl md:rounded-[2.5rem] p-8 md:p-12 flex flex-col items-center justify-center text-center cursor-pointer transition-all group ${importFile ? 'border-emerald-400 bg-emerald-50/50' : 'border-blue-300 bg-blue-50/30 hover:border-blue-500 hover:bg-blue-50'}`}>
                 <div className="p-3 md:p-5 bg-white rounded-full md:rounded-[1.5rem] shadow-sm border border-slate-200 mb-3 md:mb-4 group-hover:scale-110 transition-all duration-300">
                    {importFile ? <CheckCircle2 className="w-8 h-8 md:w-10 md:h-10 text-emerald-500" /> : <FileSpreadsheet className="w-8 h-8 md:w-10 md:h-10 text-blue-600" />}
                 </div>
                 ...
                 {!importFile && <span className="text-xs md:text-sm font-medium text-slate-500 px-2">Klik di sini untuk menelusuri file dari komputer Anda</span>}
               ...
               {previewData.length > 0 && (
                 <div className="border border-slate-200 rounded-xl md:rounded-[1.5rem] overflow-hidden shadow-sm animate-in slide-in-from-bottom-2">
                   <div className="flex justify-between items-center p-4 md:p-5 bg-slate-50 border-b border-slate-100">
                      <p className="text-[10px] md:text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-1.5 md:gap-2"><LayoutList className="w-3.5 h-3.5 md:w-4 md:h-4 text-slate-400"/> Preview Data <span className="hidden sm:inline">({previewData.length} Ruangan)</span></p>
                   ...
                   <div className="overflow-x-auto max-h-64 md:max-h-80 custom-scrollbar">
                     <table className="w-full text-xs md:text-sm text-left whitespace-nowrap">
                       <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-black text-[9px] md:text-[10px] uppercase tracking-widest sticky top-0 z-10 shadow-sm">
                         <tr><th className="p-3 md:p-4 px-4 md:px-6">Nama Ruangan</th><th className="p-3 md:p-4 px-4 md:px-6">Mapel Ujian</th><th className="p-3 md:p-4 px-4 md:px-6">Kapasitas</th><th className="p-3 md:p-4 px-4 md:px-6">Pengawas</th><th className="p-3 md:p-4 px-4 md:px-6">Plot Siswa</th></tr>
                       </thead>
                         ...
                         {previewData.map((r, i) => (
                           <tr key={i} className={r.isDuplicate ? "bg-rose-50/50" : "bg-white hover:bg-slate-50/50"}>
                             <td className={`p-3 md:p-4 px-4 md:px-6 font-bold ${r.isDuplicate ? "text-rose-700" : "text-slate-800"}`}>
                               {r.room_name} {r.isDuplicate && <span className="ml-1.5 md:ml-2 text-[8px] md:text-[9px] font-black text-rose-500 border border-rose-200 bg-rose-100 px-1.5 py-0.5 rounded md:rounded-md whitespace-nowrap">SUDAH ADA</span>}
                             </td>
                             <td className={`p-3 md:p-4 px-4 md:px-6 font-bold ${r.isDuplicate ? "text-rose-600" : "text-blue-700"}`}>{r.subject}</td>
                             <td className="p-3 md:p-4 px-4 md:px-6 font-medium text-slate-700">{r.capacity} Orang</td>
                             <td className="p-3 md:p-4 px-4 md:px-6 font-medium text-blue-600">{r.proctor_ids.length} Guru</td>
                             <td className="p-3 md:p-4 px-4 md:px-6 font-mono font-bold text-emerald-600">{r.nis_list.length} Siswa</td>
                           </tr>
                         ))}
                       </tbody>
                     </table>
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-xl rounded-2xl md:rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[95vh] md:max-h-[90vh]">
            <div className="p-4 md:p-6 lg:p-8 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50/80 gap-3">
               <div className="min-w-0">
                 <h2 className="text-lg md:text-xl font-black text-slate-800 flex items-center gap-2 truncate">
                   ...
                   <span className="truncate">{editingId ? 'Edit Sesi Ruang Ujian' : 'Tambah Sesi Ruang Baru'}</span>
                 </h2>
               </div>
               <button onClick={() => setIsFormOpen(false)} className="p-1.5 md:p-2 bg-white text-slate-400 hover:text-rose-500 border border-slate-200 rounded-full transition-colors shadow-sm shrink-0"><X className="w-4 h-4 md:w-5 md:h-5"/></button>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto custom-scrollbar flex flex-col">
               <div className="p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6 flex-1">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
                   <div>
                     <label className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest block mb-1.5 md:mb-2 flex items-center gap-1.5 md:gap-2"><Building className="w-3.5 h-3.5 md:w-4 md:h-4"/> Nama Ruangan Fisik *</label>
                   ...
                   <div>
                     <label className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest block mb-1.5 md:mb-2 flex items-center gap-1.5 md:gap-2"><Users className="w-3.5 h-3.5 md:w-4 md:h-4"/> Kapasitas Siswa *</label>
                 ...
                 <div className="relative" ref={examDropdownRef}>
                    <label className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest block mb-1.5 md:mb-2 flex items-center gap-1.5 md:gap-2"><BookOpen className="w-3.5 h-3.5 md:w-4 md:h-4"/> Jadwal Mata Pelajaran Ujian *</label>
                    <div 
                    ...
                    >
                       <span className={`truncate pr-4 ${formData.subject ? 'text-slate-800' : 'text-slate-400 font-medium'}`}>
                          {formData.subject || 'Pilih Jadwal Ujian...'}
                       </span>
                       <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform shrink-0 ${showExamDropdown ? 'rotate-180' : ''}`} />
                    </div>

                    {showExamDropdown && (
                       <div className="absolute z-10 w-full mt-1.5 md:mt-2 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden flex flex-col max-h-64 md:max-h-72">
                          <div className="p-2.5 md:p-3 border-b border-slate-100 bg-slate-50 sticky top-0 z-10">
                             <div className="relative">
                                ...
                                <input 
                                   ...
                                   className="w-full bg-white border border-slate-200 rounded-lg pl-8 md:pl-9 pr-3 py-2 text-xs md:text-sm focus:outline-none focus:border-blue-400 shadow-sm transition-colors"
                                />
                             </div>
                          </div>
                          <div className="overflow-y-auto p-1.5 md:p-2 space-y-1 custom-scrollbar bg-white">
                                ...
                                filteredExamsList.map(exam => {
                                   ...
                                   return (
                                      <div 
                                         ...
                                         className="p-2.5 md:p-3 hover:bg-blue-50 rounded-lg cursor-pointer border border-transparent hover:border-blue-100 transition-colors group"
                                      >
                                         <p className="font-bold text-slate-800 text-xs md:text-sm group-hover:text-blue-700 transition-colors">{exam.subject}</p>
                                         <div className="flex flex-wrap items-center gap-1.5 md:gap-2 mt-1 md:mt-1.5">
                                            <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest bg-slate-100 text-slate-600 px-1.5 md:px-2 py-0.5 rounded whitespace-nowrap">{exam.grade_level || 'Umum'}</span>
                                            <span className="text-[8px] md:text-[10px] font-bold text-slate-500 bg-white border border-slate-200 px-1.5 md:px-2 py-0.5 rounded truncate max-w-[150px] md:max-w-[200px]"><UserCircle2 className="w-2.5 h-2.5 md:w-3 md:h-3 inline mr-0.5 md:mr-1 -mt-0.5"/> {pengampuText}</span>
                                         </div>
                                      </div>
                                   );
                                })
                 ...
                 <div className="bg-blue-50/30 border border-blue-100 rounded-xl md:rounded-2xl p-4 md:p-5 mt-2 md:mt-4 shadow-sm">
                    <label className="text-[10px] md:text-xs font-black text-blue-600 uppercase tracking-widest block mb-2 md:mb-3 flex items-center justify-between">
                       <span className="flex items-center gap-1.5 md:gap-2"><ShieldCheck className="w-3.5 h-3.5 md:w-4 md:h-4"/> Pengawas Sesi Ini</span>
                       <span className="text-[9px] md:text-[10px] bg-white text-blue-700 px-1.5 md:px-2 py-0.5 md:py-1 rounded-md border border-blue-200 shadow-sm">{formData.proctor_ids.length} Dipilih</span>
                    </label>
                    <p className="text-[10px] md:text-xs text-blue-800/70 font-medium mb-3 leading-snug">Cari dan centang guru yang bertugas mengawasi ruangan ini.</p>
                    ...
                    <div className="relative mb-3">
                       ...
                       <input 
                          ...
                          className="w-full bg-white border border-slate-200 rounded-lg md:rounded-xl pl-8 md:pl-9 pr-3 md:pr-4 py-2 md:py-2.5 text-xs md:text-sm font-bold text-slate-700 focus:ring-2 focus:ring-blue-500 outline-none shadow-sm transition-all" 
                       />
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 md:gap-3 max-h-32 md:max-h-40 overflow-y-auto pr-1 md:pr-2 custom-scrollbar">
                       {teachers.length === 0 ? (
                          <p className="text-[10px] md:text-xs italic text-slate-400 col-span-1 sm:col-span-2 bg-white p-3 rounded-lg md:rounded-xl border border-slate-200 text-center">Belum ada data guru/pengawas di sistem.</p>
                       ) : filteredTeachersForModal.length === 0 ? (
                          <p className="text-[10px] md:text-xs italic text-slate-400 col-span-1 sm:col-span-2 text-center py-2">Pengawas tidak ditemukan.</p>
                       ) : (
                          filteredTeachersForModal.map(teacher => (
                             <label key={teacher.id} className={`flex items-center gap-2 md:gap-3 p-2.5 md:p-3 rounded-lg md:rounded-xl border-2 cursor-pointer transition-all ${formData.proctor_ids.includes(teacher.id) ? 'bg-white border-blue-500 shadow-sm' : 'bg-white/50 border-slate-200 hover:border-blue-300'}`}>
                               <input type="checkbox" checked={formData.proctor_ids.includes(teacher.id)} onChange={() => handleProctorToggle(teacher.id)} className="w-3.5 h-3.5 md:w-4 md:h-4 text-blue-600 rounded border-slate-300 focus:ring-blue-500 cursor-pointer shrink-0 shadow-sm" />
                               <span className={`text-xs md:text-sm font-bold leading-tight truncate ${formData.proctor_ids.includes(teacher.id) ? 'text-blue-800' : 'text-slate-600'}`}>{teacher.full_name}</span>
                             </label>
                          ))
                       )}
                    </div>
               ...
               <div className="p-4 md:p-6 lg:p-8 border-t border-slate-100 bg-white flex gap-2.5 md:gap-3 shrink-0 mt-auto">
                 <button type="button" onClick={() => setIsFormOpen(false)} className="flex-1 py-3 md:py-3.5 bg-slate-50 border border-slate-200 text-slate-600 font-bold rounded-lg md:rounded-xl hover:bg-slate-100 transition-colors shadow-sm text-xs md:text-sm">Batal</button>
                 <button type="submit" disabled={isSubmitting} className="flex-1 py-3 md:py-3.5 bg-blue-600 text-white font-bold rounded-lg md:rounded-xl hover:bg-blue-700 shadow-md shadow-blue-200 active:scale-95 transition-all disabled:bg-slate-300 flex items-center justify-center gap-1.5 md:gap-2 text-xs md:text-sm">
                   {isSubmitting ? <LoaderCircle className="w-4 h-4 md:w-5 md:h-5 animate-spin"/> : <Save className="w-4 h-4 md:w-5 md:h-5"/>} Simpan <span className="hidden sm:inline">Sesi Ruangan</span>
                 </button>
               </div>
            </form>
          </div>
      ...
      {viewMode === 'master' ? (
                          ...
                          <tr key={room.id} className="hover:bg-blue-50/30 transition-colors group">
                            ...
                            <td className="px-6 lg:px-8 py-4 md:py-5 text-right">
                              <div className="flex items-center justify-end gap-1.5 md:gap-2 transition-opacity">
                                <button onClick={() => {setActiveRoom(room); setViewMode('plotting'); setPlotSearchQuery('');}} className="p-2 md:p-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-full transition-all shadow-md flex items-center justify-center shrink-0" title="Atur Siswa di Ruangan Ini">
                                ...
                                <button onClick={() => openEditForm(room)} className="p-2 md:p-2.5 bg-white hover:bg-slate-100 text-slate-400 hover:text-slate-700 rounded-full transition-all border border-slate-200 shadow-sm shrink-0" title="Edit Sesi Ruangan"><Edit3 className="w-3.5 h-3.5 md:w-4 md:h-4"/></button>
                                <button onClick={() => handleDelete(room.id, room.room_name)} className="p-2 md:p-2.5 bg-white hover:bg-rose-50 text-slate-400 hover:text-rose-600 rounded-full transition-all border border-slate-200 hover:border-rose-200 shadow-sm shrink-0" title="Hapus Ruangan"><Trash2 className="w-3.5 h-3.5 md:w-4 md:h-4"/></button>
                              </div>
                            </td>
                          </tr>
                        );
                      ...
                      return (
                        <div key={room.id} className="p-4 flex flex-col gap-3 hover:bg-blue-50/30 transition-colors">
                           ...
                           <div className="grid grid-cols-5 gap-2 mt-1">
                              <button onClick={() => {setActiveRoom(room); setViewMode('plotting'); setPlotSearchQuery('');}} className="col-span-3 p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all shadow-md flex items-center justify-center gap-1.5 text-[10px] font-bold">
                                 <UserPlus className="w-3.5 h-3.5"/> Atur Siswa
                              </button>
                              <div className="col-span-2 flex gap-2">
                                <button onClick={() => openEditForm(room)} className="flex-1 p-2 bg-white hover:bg-slate-100 text-slate-500 hover:text-slate-700 rounded-lg transition-all border border-slate-200 shadow-sm flex items-center justify-center">
                                   <Edit3 className="w-3.5 h-3.5"/>
                                </button>
                                <button onClick={() => handleDelete(room.id, room.room_name)} className="flex-1 p-2 bg-white hover:bg-rose-50 text-slate-500 hover:text-rose-600 rounded-lg transition-all border border-slate-200 hover:border-rose-200 shadow-sm flex items-center justify-center">
                           ...
                           <div className="flex items-center gap-2 mt-1">
                              <button onClick={() => handleDownloadPDF(room)} disabled={isGeneratingPdf} className="flex-1 p-2 bg-white hover:bg-blue-50 text-slate-500 hover:text-blue-600 rounded-lg transition-all border border-slate-200 hover:border-blue-200 shadow-sm disabled:opacity-50 flex items-center justify-center gap-1.5 text-[10px] font-bold">
                                 {isGeneratingPdf ? <LoaderCircle className="w-3.5 h-3.5 animate-spin"/> : <FileText className="w-3.5 h-3.5"/>} PDF Hadir
                              </button>
                              <button onClick={() => handleDownloadExcel(room)} className="flex-1 p-2 bg-white hover:bg-emerald-50 text-slate-500 hover:text-emerald-600 rounded-lg transition-all border border-slate-200 hover:border-emerald-200 shadow-sm flex items-center justify-center gap-1.5 text-[10px] font-bold">
         ...
         <div className="space-y-4 md:space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <button onClick={() => setViewMode('master')} className="flex items-center justify-center sm:justify-start gap-1.5 md:gap-2 text-xs md:text-sm font-bold text-slate-500 hover:text-blue-600 bg-white border border-slate-200 hover:border-blue-200 px-4 md:px-5 py-2.5 md:py-2.5 rounded-xl transition-all w-full sm:w-fit shadow-sm">
              <ArrowLeft className="w-4 h-4 md:w-5 md:h-5" /> <span className="hidden sm:inline">Kembali ke Manajemen Sesi Ruangan</span><span className="sm:hidden">Kembali</span>
            </button>
            ...
            {activeRoom && (
              <div className="rounded-2xl md:rounded-[2.5rem] p-5 sm:p-6 md:p-10 shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6 bg-gradient-to-br from-blue-600 to-indigo-700 relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 md:gap-5 relative z-10 w-full md:w-auto min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-white/80 text-[9px] md:text-xs uppercase tracking-widest mb-0.5 md:mb-1.5 truncate">Penempatan Peserta - {activeRoom.subject || 'Mapel Umum'}</p>
                    <h2 className="text-2xl sm:text-3xl md:text-4xl font-black text-white tracking-tight truncate leading-tight">{activeRoom.room_name}</h2>
                  </div>
                </div>
                <div className="flex flex-row md:flex-col items-center md:items-end justify-between w-full md:w-auto relative z-10 bg-white/10 md:bg-transparent p-3 md:p-0 rounded-xl md:rounded-none">
                   <span className="text-[9px] md:text-xs font-bold text-white/80 uppercase tracking-widest mb-0 md:mb-1.5">Kapasitas Terisi</span>
                   <div className="md:bg-white/20 md:px-5 md:py-2.5 md:rounded-xl md:backdrop-blur-md md:border md:border-white/30 text-white">
                      <span className="text-xl sm:text-2xl md:text-3xl font-black">{getRoomOccupancy(activeRoom.id)}</span><span className="text-xs md:text-sm font-bold ml-1">/ {activeRoom.capacity} Siswa</span>
                   </div>
                </div>
              </div>
            )}

            {/* PERBAIKAN: Di HP, Grid Plotting menjadi flex-col agar atas bawah */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
               
               {/* BAGIAN KIRI/ATAS: DAFTAR SISWA YANG BISA DIMASUKKAN */}
               <div className="bg-white border border-slate-200 rounded-2xl md:rounded-[2rem] shadow-sm flex flex-col h-[500px] sm:h-[600px] md:h-[700px] overflow-hidden order-2 lg:order-1">
                  <div className="p-4 md:p-6 border-b border-slate-100 bg-slate-50/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shrink-0">
                     <div>
                       <h3 className="font-black text-slate-800 flex items-center gap-1.5 md:gap-2 text-sm md:text-base"><Users className="w-4 h-4 md:w-5 md:h-5 text-blue-600"/> Siswa Belum Dapat Sesi</h3>
                       <p className="text-[10px] md:text-xs font-medium text-slate-500 mt-0.5 md:mt-1">Pilih siswa untuk dimasukkan ke sesi ini.</p>
                     </div>
                     <span className="bg-blue-100 text-blue-700 font-black text-[10px] md:text-xs px-2.5 md:px-3 py-1 md:py-1.5 rounded-lg self-start sm:self-auto">{availableStudents.length} Siswa</span>
                  </div>
                  <div className="p-3 md:p-5 shrink-0 border-b border-slate-100">
                    <div className="relative">
                      <Search className="absolute left-3.5 md:left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                      <input type="text" placeholder="Cari nama atau kelas siswa..." value={plotSearchQuery} onChange={(e) => setPlotSearchQuery(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-lg md:rounded-xl pl-10 md:pl-11 pr-3 md:pr-4 py-2.5 md:py-3 text-xs md:text-sm font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 outline-none transition-all placeholder-slate-400" />
                    </div>
                  </div>
                  <div className="flex-1 overflow-y-auto p-3 md:p-4 custom-scrollbar bg-slate-50/30">
                     {availableStudents.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-slate-400 p-4 md:p-6 text-center">
                           <div className="w-16 h-16 md:w-20 md:h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto border border-slate-100 mb-3 md:mb-4"><CheckCircle2 className="w-8 h-8 md:w-10 md:h-10 text-slate-300"/></div>
                           <p className="font-bold text-slate-500 text-sm md:text-base">Semua siswa sudah di-plot ke ruangan.</p>
                        </div>
                     ) : (
                        <div className="space-y-1.5 md:space-y-2">
                           {availableStudents.map(student => (
                              <div key={student.id} className="flex items-center justify-between p-3 md:p-3.5 rounded-xl md:rounded-2xl bg-white hover:bg-blue-50 border border-slate-200 hover:border-blue-200 transition-all shadow-sm group">
                                 <div className="min-w-0 pr-2">
                                   <p className="font-black text-slate-800 text-xs md:text-sm leading-tight mb-1 truncate group-hover:text-blue-700 transition-colors">{student.full_name}</p>
                                   <div className="flex gap-1.5 md:gap-2 mt-0.5 md:mt-1">
                                      <span className="text-[8px] md:text-[10px] font-black uppercase tracking-widest bg-slate-100 text-slate-500 px-1.5 md:px-2 py-0.5 rounded border border-slate-200 truncate max-w-[80px] sm:max-w-none">NIS: {student.student_number}</span>
                                 ...
                                 <button onClick={() => handleAssignStudent(student.id)} disabled={getRoomOccupancy(activeRoom?.id || '') >= (activeRoom?.capacity || 0)} className="w-8 h-8 md:w-10 md:h-10 shrink-0 flex items-center justify-center rounded-full bg-slate-50 text-slate-400 hover:bg-blue-600 hover:text-white border border-slate-200 hover:border-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
               ...
               {/* BAGIAN KANAN/BAWAH: DAFTAR SISWA DI DALAM RUANGAN INI */}
               <div className="bg-white border border-slate-200 rounded-2xl md:rounded-[2rem] shadow-sm flex flex-col h-[500px] sm:h-[600px] md:h-[700px] overflow-hidden order-1 lg:order-2">
                  <div className="p-4 md:p-6 border-b border-slate-100 bg-emerald-50/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 shrink-0">
                     <div>
                       ...
                       <p className="text-[10px] md:text-xs font-medium text-slate-500 mt-0.5 md:mt-1">Siswa yang telah di-plot ke {activeRoom?.room_name}.</p>
                     </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-3 md:p-4 custom-scrollbar bg-slate-50/30">
                     {studentsInRoom.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-slate-400 p-4 md:p-6 text-center">
                           ...
                           <p className="text-[10px] md:text-xs font-medium mt-1">Pindahkan siswa dari daftar di sebelah kiri/bawah.</p>
                        </div>
                     ) : (
                        <div className="space-y-1.5 md:space-y-2">
                           {studentsInRoom.map((student, idx) => (
                              <div key={student.id} className="flex items-center justify-between p-3 md:p-3.5 rounded-xl md:rounded-2xl bg-white border border-slate-200 shadow-sm hover:border-rose-300 transition-all group animate-in fade-in slide-in-from-left-2">
                                 ...
                                 <button onClick={() => handleRemoveStudent(student.id)} title="Keluarkan dari ruangan" className="w-8 h-8 md:w-10 md:h-10 shrink-0 flex items-center justify-center rounded-full bg-white text-slate-400 hover:bg-rose-600 hover:text-white hover:border-rose-600 border border-slate-200 transition-colors shadow-sm">
                  ...
                  {/* Footer Aksi Ruangan */}
                  <div className="p-4 md:p-6 border-t border-slate-100 bg-white shrink-0 flex flex-col sm:flex-row justify-end gap-2.5 md:gap-3">
                     <button onClick={() => handleDownloadPDF(activeRoom!)} disabled={isGeneratingPdf} className="flex items-center justify-center gap-1.5 md:gap-2 py-3 md:py-3.5 px-4 md:px-6 bg-white border-2 border-slate-200 hover:border-blue-500 hover:bg-blue-50 text-slate-700 hover:text-blue-700 font-bold rounded-lg md:rounded-xl transition-all text-xs md:text-sm disabled:opacity-50 w-full sm:w-auto">
                     ...
                     <button onClick={() => handleDownloadExcel(activeRoom!)} className="flex items-center justify-center gap-1.5 md:gap-2 py-3 md:py-3.5 px-4 md:px-6 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-bold rounded-lg md:rounded-xl transition-colors text-xs md:text-sm border border-emerald-200 shadow-sm w-full sm:w-auto"><FileSpreadsheet className="w-4 h-4 md:w-5 md:h-5"/> Unduh Excel</button>
                  </div>
               </div>
            </div>
         </div>
