...
export default function TeachersManagementPage() {
  ...
  const inputClass = "w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition-all placeholder:text-slate-400";
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 p-4 md:p-8 max-w-7xl mx-auto text-slate-900 relative pb-24">
         ...
         <div className="fixed inset-0 z-[130] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col border border-slate-100">
               <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                  <div>
                    <h3 className="text-xl font-black text-slate-800 flex items-center gap-2"><Printer className="w-6 h-6 text-blue-600"/> Unduh Data Guru</h3>
               ...
               <div className="p-8 space-y-4">
                  <button onClick={executeDownloadPDF} disabled={isGeneratingPdf} className="w-full flex items-center p-5 rounded-2xl border-2 border-slate-200 hover:border-blue-500 hover:bg-blue-50 transition-all group text-left disabled:opacity-70 disabled:cursor-not-allowed">
                     ...
                     <div>
                       ...
                       <p className="text-xs text-slate-500 font-medium mt-1 leading-relaxed">Tabel siap cetak, rapi, dan dilengkapi kolom tanda tangan.</p>
                     </div>
                  </button>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 md:px-8 md:py-6 rounded-[2rem] border border-blue-100 shadow-sm">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-slate-800 flex items-center gap-3">
            <GraduationCap className="w-8 h-8 text-blue-600" /> Data Guru
          </h1>
          <p className="text-slate-500 text-sm mt-1 font-medium ml-11">Kelola data pengajar, akses login, dan penugasan mata pelajaran.</p>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={openCreateForm} className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold text-sm shadow-md shadow-blue-200 active:scale-95 w-full md:w-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 mt-2">
        ...
        <button onClick={() => setIsPrintOptionsOpen(true)} className="flex items-center justify-center gap-2 bg-white hover:bg-slate-50 text-blue-700 border border-slate-200 hover:border-blue-200 px-6 py-3.5 rounded-[1.5rem] font-bold text-sm shadow-sm transition-colors shrink-0">
           <Download className="w-5 h-5 text-blue-600" /> Unduh Akun Guru
        </button>
      </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50/80 text-slate-500 text-[11px] font-black uppercase tracking-widest border-b border-slate-100">
              <tr>
                ...
                <th className="px-8 py-5">Profil Guru</th>
                <th className="px-8 py-5">Mata Pelajaran Diampu</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                <tr><td colSpan={5} className="py-24 text-center"><LoaderCircle className="w-10 h-10 text-blue-500 animate-spin mx-auto mb-3" /><p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Memuat data guru...</p></td></tr>
              ) : filteredTeachers.length === 0 ? (
                <tr><td colSpan={5} className="py-24 text-center"><div className="w-20 h-20 bg-slate-50 rounded-full flex items-center justify-center mx-auto border border-slate-100 mb-4"><GraduationCap className="w-10 h-10 text-slate-300" /></div><p className="text-slate-500 font-bold text-lg">Data guru tidak ditemukan.</p></td></tr>
              ) : (
                filteredTeachers.map((teacher, idx) => (
                  <tr key={teacher.id} className="hover:bg-blue-50/30 transition-colors group">
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                         {teacher.avatar_url ? (
                           <img src={getAvatarUrl(teacher.avatar_url)} alt="Avatar" className="w-12 h-12 rounded-[1.2rem] object-cover border border-slate-200 shadow-sm shrink-0" />
                         ) : (
                           <div className="w-12 h-12 rounded-[1.2rem] bg-blue-50 flex items-center justify-center text-blue-600 font-black text-lg border border-blue-100 shrink-0">
                              {teacher.full_name.charAt(0).toUpperCase()}
                           </div>
                         )}
                         <div>
                            <p className="font-black text-slate-800 text-base leading-tight mb-1">{teacher.full_name}</p>
                            <span className="inline-flex items-center gap-1.5 text-[9px] font-black bg-indigo-50 text-indigo-600 px-2.5 py-1 rounded-md border border-indigo-100 uppercase tracking-widest">
                    ...
                    <td className="px-8 py-5">
                       <div className="flex flex-wrap gap-1.5 max-w-[250px]">
                          {(!teacher.taught_subjects || teacher.taught_subjects.length === 0) ? (
                             <span className="text-xs font-bold text-slate-400 italic">Belum memegang mapel</span>
                          ) : (
                             teacher.taught_subjects.map(subjId => {
                                const subj = subjectsList.find(s => s.id === subjId);
                                if (!subj) return null;
                                return (
                                   <div key={subjId} className="bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">
                                      <p className="text-xs font-bold text-slate-700 leading-none">{subj.name}</p>
                                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">{subj.grade_level}</p>
                                   </div>
                                );
                             })
                          )}
                    ...
                    <td className="px-8 py-5">
                       <div className="space-y-1.5">
                          <p className="text-sm font-bold text-slate-700 flex items-center gap-2"><UserCircle2 className="w-4 h-4 text-slate-400"/> {teacher.username}</p>
                          {teacher.password && <p className="text-[11px] font-mono font-bold text-slate-500 flex items-center gap-2 bg-slate-100 w-fit px-2.5 py-1 rounded-md border border-slate-200"><KeyRound className="w-3 h-3"/> {teacher.password}</p>}
                       </div>
                    </td>
                    <td className="px-8 py-5 text-right">
                      <div className="flex items-center justify-end gap-2 transition-opacity">
                        <button onClick={() => openEditForm(teacher)} className="p-2.5 bg-white hover:bg-blue-50 text-slate-400 hover:text-blue-600 rounded-full transition-all border border-slate-200 hover:border-blue-200 shadow-sm" title="Edit Data"><Edit3 className="w-4 h-4"/></button>
                        <button onClick={() => handleDelete(teacher.id, teacher.full_name)} className="p-2.5 bg-white hover:bg-rose-50 text-slate-400 hover:text-rose-600 rounded-full transition-all border border-slate-200 hover:border-rose-200 shadow-sm" title="Hapus Guru"><Trash2 className="w-4 h-4"/></button>
                      </div>
                    </td>
                  </tr>
                ))
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto">
               <div className="p-6 md:p-8 space-y-6">
                 ...
                 <div className="space-y-4">
                    <div>
                      <label className="text-xs font-black text-slate-500 uppercase tracking-widest block mb-2 flex items-center gap-2"><UserCircle2 className="w-4 h-4"/> Nama Lengkap *</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-black text-slate-500 uppercase tracking-widest block mb-2 flex items-center gap-2"><GraduationCap className="w-4 h-4"/> NIP (Atau Username) *</label>
                      ...
                      <div>
                        <label className="text-xs font-black text-slate-500 uppercase tracking-widest block mb-2 flex items-center gap-2"><KeyRound className="w-4 h-4"/> Password *</label>
                 ...
                 <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col h-full">
                    <div className="flex items-center justify-between mb-3">
                       <label className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                          <BookOpen className="w-4 h-4"/> Mata Pelajaran Diampu
                       </label>
                       <span className="text-[10px] font-bold bg-blue-100 text-blue-700 px-2 py-1 rounded-md">{formData.taught_subjects.length} Dipilih</span>
                    </div>
                    ...
                    <div className="relative mb-3">
                      ...
                      <input 
                        ...
                        className="w-full bg-white border border-slate-200 rounded-xl pl-9 pr-3 py-2 text-xs font-medium text-slate-700 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                      />
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                       {filteredSubjectsModal.length === 0 ? (
                          <p className="text-xs italic text-slate-400 col-span-2 py-2 text-center">Mata pelajaran tidak ditemukan.</p>
                       ) : (
                          filteredSubjectsModal.map(subj => (
                             <label key={subj.id} className={`flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${formData.taught_subjects.includes(subj.id) ? 'bg-blue-50 border-blue-500' : 'bg-white border-slate-200 hover:border-blue-300'}`}>
                               <input type="checkbox" checked={formData.taught_subjects.includes(subj.id)} onChange={() => handleSubjectToggle(subj.id)} className="w-4 h-4 mt-0.5 text-blue-600 rounded focus:ring-blue-500 cursor-pointer" />
                               <div>
                                 <p className={`text-sm font-bold leading-tight ${formData.taught_subjects.includes(subj.id) ? 'text-blue-800' : 'text-slate-700'}`}>{subj.name}</p>
                                 <p className={`text-[10px] font-black uppercase tracking-widest mt-1 ${formData.taught_subjects.includes(subj.id) ? 'text-blue-500' : 'text-slate-400'}`}>{subj.grade_level}</p>
                               </div>
                             </label>
                          ))
                       )}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-4xl rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            <div className="p-6 md:p-8 border-b border-slate-100 bg-slate-50/80 flex justify-between items-center shrink-0">
               <h3 className="text-xl font-black text-slate-800 flex items-center gap-3"><FileUp className="w-6 h-6 text-blue-600"/> Import Massal Data Guru</h3>
               <button onClick={() => {setIsImportOpen(false); setPreviewData([]); setImportFile(null);}} className="p-2 bg-white text-slate-400 hover:text-rose-500 border border-slate-200 rounded-full transition-colors shadow-sm"><X className="w-5 h-5"/></button>
            </div>
                   ...
                   <div className="overflow-x-auto max-h-80 custom-scrollbar">
                     <table className="w-full text-sm text-left">
                       <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-black text-[10px] uppercase tracking-widest sticky top-0 z-10 shadow-sm">
                         <tr><th className="p-4 px-6">Nama Lengkap</th><th className="p-4 px-6">NIP / Username</th><th className="p-4 px-6">Password</th><th className="p-4 px-6">Mapel Diampu</th></tr>
                       </thead>
                       <tbody className="divide-y divide-slate-100">
                         {previewData.slice(0, 50).map((r, i) => (
                           <tr key={i} className={r.isDuplicate ? "bg-rose-50/50" : "bg-white hover:bg-slate-50/50"}>
                             ...
                             <td className={`p-4 px-6 font-medium ${r.isDuplicate ? "text-rose-600" : "text-slate-600"}`}>{r.username}</td>
                             <td className={`p-4 px-6 font-mono font-bold ${r.isDuplicate ? "text-rose-400" : "text-emerald-600"}`}>{r.isDuplicate ? "--------" : r.password}</td>
                             <td className={`p-4 px-6 font-bold ${r.isDuplicate ? "text-rose-400" : "text-blue-600"}`}>{r.taught_subjects.length} Mapel</td>
                           </tr>
                         ))}
            ...
            <div className="p-6 md:p-8 border-t border-slate-100 bg-slate-50/80 flex gap-3 shrink-0">
               <button onClick={() => {setIsImportOpen(false); setPreviewData([]); setImportFile(null);}} className="flex-1 py-3.5 bg-white border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-100 transition-colors shadow-sm">Batal</button>
