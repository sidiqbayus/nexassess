...
const SmartMediaRenderer = ({ url, className = "" }: { url?: string, className?: string }) => {
  ...
  if (ytMatch) return (<div className={`relative w-full max-w-xl mx-auto aspect-video rounded-xl md:rounded-2xl overflow-hidden shadow-sm border border-slate-200 mt-3 md:mt-4 ${className}`}><iframe src={`https://www.youtube.com/embed/${ytMatch[1]}?modestbranding=1&rel=0`} className="w-full h-full border-0" /></div>);
  if (url.match(/\.(mp4|webm|ogg)$/i)) return (<div className={`relative w-full max-w-xl mx-auto aspect-video rounded-xl md:rounded-2xl overflow-hidden shadow-sm border border-slate-200 mt-3 md:mt-4 bg-black ${className}`}><video src={url} controls className="w-full h-full" /></div>);
...
const processHtmlMedia = (html: string) => {
  ...
  p = p.replace(/(?:<a[^>]*href="|>|\s|^)(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:[^\/]+\/.+\/|(?:v|e(?:mbed)?)\/|.*[?&]v=)|youtu\.be\/)([^"&?\/\s<]{11})[^<]*(?:<\/a>|<|\s|$)/g, (match, videoId) => `<div class="relative w-full max-w-2xl mx-auto aspect-video rounded-xl md:rounded-2xl overflow-hidden shadow-sm border border-slate-200 my-4"><iframe src="https://www.youtube.com/embed/${videoId}?modestbranding=1&rel=0&controls=0&fs=0&disablekb=1" class="w-full h-full border-0"></iframe></div>`);
  p = p.replace(/<a[^>]*href="((?:https?:\/\/)?drive\.google\.com\/(?:file\/d\/|open\?id=|uc\?id=|uc\?export=view&id=)([a-zA-Z0-9_-]+)[^"]*)"[^>]*>(.*?)<\/a>/gi, (match, fullUrl, driveId, text) => {
      if (text.toUpperCase().includes('AUDIO') || text.toUpperCase().includes('MP3') || text.toUpperCase().includes('SUARA')) return `<div class="my-4 p-3 bg-blue-50 border border-blue-200 rounded-xl md:rounded-2xl w-full max-w-xl"><span class="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1 flex items-center gap-1">🎧 Audio</span><audio src="https://docs.google.com/uc?export=open&id=${driveId}" controls controlsList="nodownload" class="w-full h-10 outline-none mt-1"></audio></div>`;
      return `<div class="relative w-full max-w-xl aspect-video rounded-xl md:rounded-2xl overflow-hidden shadow-sm border border-slate-200 my-4"><iframe src="https://drive.google.com/file/d/${driveId}/preview" class="w-full h-full border-0"></iframe></div>`;
  });
  p = p.replace(/<a[^>]*href="([^"]+\.(?:mp3|wav|m4a))"[^>]*>.*?<\/a>/gi, '<div class="my-4 p-3 bg-blue-50 border border-blue-200 rounded-xl md:rounded-2xl w-full max-w-xl"><span class="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1 flex items-center gap-1">🎧 Audio Jawaban Siswa</span><audio src="$1" controls controlsList="nodownload" class="w-full h-10 outline-none mt-1"></audio></div>');
  p = p.replace(/<a[^>]*href="([^"]+\.(?:mp4|webm|ogg))"[^>]*>.*?<\/a>/gi, `<div class="relative w-full max-w-xl aspect-video rounded-xl md:rounded-2xl overflow-hidden shadow-sm border border-slate-200 my-4 bg-black"><video src="$1" controls controlsList="nodownload nofullscreen" class="w-full h-full"></video></div>`);
  return p;
};
...
interface StudentGroupedResult {
  student_id: string; 
  users: { full_name: string; student_number: string; class_group: string; avatar_url?: string; };
...
export default function ReportsAndAnalyticsPage() {
  ...
  return (
    <div className="p-4 md:p-8 space-y-4 md:space-y-6 max-w-7xl mx-auto pb-24 md:pb-20 font-sans text-slate-800">
      ...
      {activeView === 'list' ? (
        <div className="space-y-4 md:space-y-6 animate-in fade-in">
           ...
           {loadingExams ? (
             ...
             <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl md:rounded-[2rem] py-16 md:py-20 text-center flex flex-col items-center px-4">
                 <div className="w-16 h-16 md:w-20 md:h-20 bg-slate-50 rounded-full flex items-center justify-center mb-3 md:mb-4"><BarChart3 className="w-8 h-8 md:w-10 md:h-10 text-slate-300"/></div>
                 <h2 className="text-lg md:text-xl font-black text-slate-700 mb-1">Tidak Ada Laporan Tersedia</h2>
                 <p className="text-slate-500 font-medium text-xs md:text-sm max-w-sm">Belum ada ujian di mata pelajaran yang ada, atau pencarian Anda tidak cocok.</p>
             </div>
           ) : (
        ...
        <div className="space-y-4 md:space-y-6 animate-in fade-in duration-300">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 md:gap-4">
             ...
             <button onClick={() => setIsPrintModalOpen(true)} className="flex items-center justify-center gap-1.5 md:gap-2 bg-white hover:bg-blue-50 hover:text-blue-700 text-slate-700 border border-slate-200 hover:border-blue-200 px-4 md:px-6 py-2.5 md:py-3 rounded-lg md:rounded-xl font-bold text-xs md:text-sm shadow-sm transition-colors w-full sm:w-auto shrink-0">
                <Printer className="w-4 h-4 md:w-5 md:h-5 text-blue-600"/> Unduh Laporan Detail
             </button>
          </div>
            ...
            <div className="bg-white w-full max-w-sm md:max-w-md rounded-2xl md:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col border border-slate-200">
               <div className="p-4 md:p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                  <div>
                    ...
                    <p className="text-xs md:text-sm font-medium text-slate-500 mt-1">Laporan lengkap untuk semua mata pelajaran.</p>
                  </div>
         ...
         <div className="fixed inset-0 z-[130] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white w-full max-w-lg rounded-2xl md:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col border border-slate-200 max-h-[90vh]">
               <div className="p-4 md:p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50 shrink-0">
                  <div className="min-w-0">
                    <h3 className="text-lg md:text-xl font-black text-slate-800 flex items-center gap-2"><Printer className="w-5 h-5 md:w-6 h-6 text-blue-600 shrink-0"/> <span className="truncate">Cetak Rekap Ujian</span></h3>
                    <p className="text-xs md:text-sm font-medium text-slate-500 mt-0.5 md:mt-1 truncate">{activeExam?.subject}</p>
                  </div>
                  <button onClick={() => setIsPrintModalOpen(false)} className="text-slate-400 hover:text-rose-500 bg-white rounded-full p-1.5 md:p-2 border border-slate-200 shadow-sm transition-colors shrink-0 ml-2"><X className="w-4 h-4 md:w-5 md:h-5"/></button>
               </div>
      ...
      {selectedStudentGroup && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4 md:p-6 animate-in fade-in zoom-in-95 duration-200">
           <div className="bg-white w-full max-w-7xl rounded-2xl md:rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden flex flex-col h-[95vh] text-slate-800">
             ...
             <div className="p-4 md:p-5 lg:p-8 border-b border-slate-100 bg-slate-50/80 flex justify-between items-start sm:items-center shrink-0">
                <div className="flex items-center gap-3 sm:gap-4 md:gap-5 min-w-0">
                   {selectedStudentGroup.users.avatar_url ? (
                      <img src={selectedStudentGroup.users.avatar_url} alt="Avatar" className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl object-cover border border-slate-200 shadow-sm shrink-0" />
                   ) : (
                   ...
                   <div className="min-w-0 flex-1">
                      <h2 className="text-base sm:text-lg md:text-xl lg:text-2xl font-black text-slate-800 flex flex-wrap items-center gap-2 md:gap-3 leading-tight">
                         <span className="truncate">{selectedStudentGroup.users.full_name}</span> 
                      </h2>
                      <p className="text-xs md:text-sm text-slate-500 mt-0.5 md:mt-1 font-bold truncate">Kelas {selectedStudentGroup.users.class_group} • Max: {selectedStudentGroup.highest_percentage.toFixed(2)}</p>
                   </div>
                </div>
                <div className="flex items-center justify-between sm:justify-end gap-2 overflow-x-auto w-full sm:w-auto custom-scrollbar hide-scroll-mobile pb-1 sm:pb-0 mt-2 sm:mt-0 shrink-0">
             ...
             <div className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 md:space-y-6 bg-slate-50/50 relative custom-scrollbar">
                {loadingReview ? (
      ...
      {selectedStudentForEssay && selectedStudentMeta && (
         <div className="fixed inset-0 z-[200] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-2 sm:p-4 md:p-8 animate-in fade-in">
            <div className="bg-white w-full max-w-5xl rounded-2xl md:rounded-[2.5rem] shadow-2xl flex flex-col h-full max-h-[95vh] overflow-hidden animate-in zoom-in-95 border border-amber-200">
