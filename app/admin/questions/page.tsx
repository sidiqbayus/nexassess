  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        allow_media_upload: qType === 'essay' ? formData.allow_media_upload : false, // Simpan status izin media
      ...
      try {
        ...
        const parsedQuestions = rawRows.map((row: any) => {
          ...
          return {
            ...
            allow_media_upload: false, // Default import false
        ...
        <>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 md:px-8 md:py-6 rounded-[2rem] border border-blue-100 shadow-sm">
            <div>
              ...
              <p className="text-slate-500 text-sm mt-1 font-medium ml-11">Pilih Folder Mata Pelajaran untuk mengelola soal secara permanen.</p>
            </div>
          ...
          {globalLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {subjects.filter(subj => subj.name.toLowerCase().includes(searchQuery.toLowerCase())).map((subject) => (
                <div key={subject.id} onClick={() => goToPackagesView(subject)} className="bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-blue-300 transition-all duration-300 cursor-pointer group flex flex-col justify-between">
                  <div>
                    <div className="w-14 h-14 bg-blue-50 border border-blue-100 rounded-[1.2rem] flex items-center justify-center mb-5 group-hover:scale-110 group-hover:bg-blue-600 transition-all duration-300 shadow-inner">
                       <FolderOpen className="w-7 h-7 text-blue-600 group-hover:text-white" />
                    </div>
                    <h3 className="font-black text-2xl text-slate-800 line-clamp-1 group-hover:text-blue-700 transition-colors" title={subject.name}>{subject.name}</h3>
                  </div>
              ...
              {subjects.filter(subj => subj.name.toLowerCase().includes(searchQuery.toLowerCase())).length === 0 && (
                 <div className="col-span-full py-10 text-center text-slate-400 font-bold">Mata Pelajaran tidak ditemukan. Silakan tambahkan di menu "Mata Pelajaran".</div>
              )}
            </div>
          )}
        </>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-50/80 text-slate-500 text-[11px] font-black uppercase tracking-widest border-b border-slate-100">
                  <tr><th className="px-8 py-5 w-12 text-center">No</th><th className="px-8 py-5">Pertanyaan</th><th className="px-8 py-5 text-center">Tipe</th><th className="px-8 py-5 text-center">Bobot</th><th className="px-8 py-5 text-center">Status</th><th className="px-8 py-5 text-right">Aksi</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {globalLoading ? (
                    <tr><td colSpan={6} className="text-center py-24"><LoaderCircle className="w-10 h-10 text-cyan-500 animate-spin mx-auto mb-3" /><p className="text-slate-400 font-bold uppercase tracking-widest text-xs">Memuat butir soal...</p></td></tr>
                  ) : questions.filter(q => (q.package_name || 'Paket 1') === activePackage).length === 0 ? (
                    <tr><td colSpan={6} className="text-center py-24 px-4"><div className="bg-slate-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-200"><AlertCircle className="w-10 h-10 text-slate-400" /></div><p className="text-slate-700 font-bold text-xl mb-1">Belum ada soal</p><p className="text-slate-500 font-medium text-sm">Klik tombol "Tambah Soal Manual" untuk menyusun paket ini.</p></td></tr>
                  ) : (
                    questions
      ...
      {isFormModalOpen && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
          <div className="min-h-screen p-4 md:p-8 flex items-center justify-center">
            <div className="bg-white w-full max-w-6xl rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-300 flex flex-col max-h-[95vh] border border-slate-200">
              
              {/* HEADER FORM */}
              <div className="bg-slate-50 border-b border-slate-100 p-6 md:p-8 flex justify-between items-center sticky top-0 z-20">
                <div>
                ...
                <div className="flex gap-3">
                   ...
                   <button onClick={handleFormSubmit} disabled={isSubmittingForm} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3.5 rounded-2xl font-bold flex items-center gap-2 shadow-md active:scale-95 disabled:opacity-70 transition-all text-sm hidden sm:flex">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-1 bg-white border border-slate-200 rounded-[2rem] p-6 md:p-8 shadow-sm space-y-5 h-fit">
                      ...
                      <div className="p-5 bg-cyan-50/50 border border-cyan-100 rounded-2xl mb-6 shadow-sm">
                        ...
                        <p className="text-[10px] font-bold text-slate-500 mb-3 leading-tight">Gunakan nama yang sama untuk menggabungkan soal dalam 1 paket (Contoh: "Paket A").</p>
                        <input 
                          type="text" 
                          name="package_name" 
                      ...
                      <div className="space-y-2">
                         <label className="text-xs font-black text-slate-500 uppercase tracking-widest mt-4 block">Tag Materi Pelajaran</label>
                         <input type="text" name="subject" value={formData.subject} onChange={handleFormChange} placeholder="Ketik topik materi di sini..." className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-blue-500 transition-all placeholder:text-slate-400 shadow-sm" />
                      </div>
                         ...
                         <div className="mt-4 p-4 bg-blue-50/50 border border-blue-200 rounded-2xl shadow-sm">
                            <label className="flex items-center gap-4 cursor-pointer">
                               ...
                               <div>
                                  ...
                                  <span className="text-[10px] font-bold text-blue-700/80 leading-tight block">Bila dicentang, siswa dapat mengunggah gambar atau menautkan link di editor jawaban mereka.</span>
                               </div>
                            </label>
                         </div>
                      )}
                    ...
                    <div className="lg:col-span-2 bg-white border border-slate-200 rounded-[2rem] p-6 md:p-8 shadow-sm">
                        ...
                        <div className="space-y-6">
                          <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100 shadow-sm flex items-start gap-4">
                             ...
                             <div>
                               ...
                               <p className="text-xs text-blue-800/80 mt-1 font-medium leading-relaxed">Anda dapat menambahkan Premis dan Respons menggunakan Editor Teks penuh. Anda juga bisa membuat jumlah Respons Kanan lebih banyak sebagai Opsi Pengecoh.</p>
                             </div>
                          </div>
                      ...
                      {(formData.question_type === 'multiple_choice' || formData.question_type === 'complex_multiple_choice') && (
                        <div className="space-y-5">
                          <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100 shadow-sm flex items-start gap-4 mb-6">
                             <Info className="w-6 h-6 text-blue-500 shrink-0 mt-0.5" />
                             <div>
                               <p className="text-sm font-bold text-blue-900">Format Pilihan Ganda</p>
                               <p className="text-xs text-blue-800/80 mt-1 font-medium leading-relaxed">{formData.question_type === 'complex_multiple_choice' ? 'Centang kotak hijau di kiri untuk menandai kunci jawaban benar (bisa lebih dari satu).' : 'Isi opsi pilihan ganda di bawah ini. Pilih Kunci Jawaban Mutlak di panel kanan.'}</p>
                             </div>
                          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 z-[90] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 border border-slate-100 flex flex-col">
            ...
            <div className="p-8 space-y-4">
              <label className="text-xs font-black text-slate-500 uppercase tracking-widest block mb-2">Mata Pelajaran Master</label>
              <div className="relative">
          ...
          <div className="bg-white w-full max-w-5xl rounded-[2.5rem] shadow-2xl flex flex-col max-h-[95vh] animate-in zoom-in-95 duration-300 overflow-hidden border border-slate-200">
            ...
            <div className="p-6 md:p-8 overflow-y-auto bg-slate-50/50 flex-1 space-y-8">
              ...
              <div className="bg-white border border-slate-200 rounded-[2rem] p-6 md:p-8 shadow-sm">
                 ...
                 <div className="mb-8">
                   <label className="font-black text-slate-800 mb-3 block text-lg flex items-center gap-2"><BookOpen className="w-5 h-5 text-slate-400"/> Pilih Target Mata Pelajaran (Master)</label>
                 ...
                 {/* AREA DROP EXCEL (DASHED BOX BESAR) */}
